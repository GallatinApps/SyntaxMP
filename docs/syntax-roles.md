...
| `SyntaxRole.String.Regex` | `string.regex` | Regex literals (JS, TS, Perl). |
...
| `SyntaxRole.Property.Name` | `property.name` | Object keys in JSON, JSON5, YAML, TOML. |
...
| `SyntaxRole.Variable.Parameter` | `variable.parameter` | Sigil variables (`$name` in PHP/Perl/Ruby), named arguments (Kotlin), bind placeholders (SQLite). |
...
| `SyntaxRole.Constant.Atom` | `constant.atom` | Elixir atoms. |
| `SyntaxRole.Markup.Expression` | `markup.expression` | Component-template expression delimiters (Vue, Svelte, Astro, MDX). |
| `SyntaxRole.Markup.Frontmatter` | `markup.frontmatter` | Astro `---` frontmatter fences. |
