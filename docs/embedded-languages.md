...
If you need a built-in host (HTML, JSX, TSX, etc.) to add new routing mechanics, such as a new raw-text tag, attribute-value scripts, or a different default for `<script>` / `<style>`, the realistic path today is to fork the host tokenizer into an extension and supply custom scanner options.
