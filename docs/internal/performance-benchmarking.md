...
near an editor-large file shape. Compact formats such as CSV, diff, DNS zone files, Dockerfiles,
...
They include TSX many-small-expression and few-large-expression shapes, JSX many-expression shape,
MDX markdown-only / JSX-only / mixed / ESM-only / fenced-code-heavy shapes, and Astro
frontmatter-heavy versus template-heavy shapes.
...
| `diagnostics/large-editor/mdx-mixed/highlight-threshold` | `mdx` | Large MDX page with Markdown, JSX islands, fenced TSX/JSON, and raw HTML. |
