...
- Built-in aliases include `js`, `ts`, `jsx`, `tsx`, `kt`, `kts`, `md`, `yml`, `tf`, `pgsql`, `sqlite3`, `htaccess`, `dotenv`, and others. They are resolved by `SyntaxTokenizerEngine`; see the source for the authoritative list.
...
When a tokenizer encounters embedded content (HTML script/style, Markdown fenced blocks, Vue/Svelte/Astro script/style regions, or a custom extension's own child regions), it can route the inner content back through the engine with `SyntaxTokenizeRequest.tokenizeEmbedded(...)`, which re-applies extension lookup. Recursion is capped at depth 3. A document with deeper nesting renders the deepest layer as plain text rather than continuing to recurse.

### Stability of `value` strings

The string value on each `SyntaxLanguageId` and `SyntaxRole` is intended to be stable across SyntaxMP releases for built-ins. Hosts may persist or transmit these strings (file format metadata, route segments, theme JSON, etc.) and round-trip them through `SyntaxTokenizerEngine.resolveLanguageId(value)` / `SyntaxRole.of(value)`.

Renaming a built-in language's `value` or a built-in role's path is a breaking change.
