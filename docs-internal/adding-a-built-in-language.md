...
This is the maintainer-facing companion for adding a **built-in** language tokenizer to SyntaxMP itself - the languages that ship in `LanguageId.BuiltIns` and live under `syntaxmp-tokenizer/src/commonMain/kotlin/com/gallatinapps/syntaxmp/builtins/`. It is not the path for host applications adding a custom extension; that audience belongs to the public [docs/language-extension.md](../docs/language-extension.md) guide.
...
```text
syntaxmp-tokenizer/src/commonMain/kotlin/com/gallatinapps/syntaxmp/language/LanguageId.kt
```
...
### 3. Wire The Tokenizer Contract

Built-in language tokenizers live under `syntaxmp-tokenizer/src/commonMain/kotlin/com/gallatinapps/syntaxmp/builtins/<language>/`.

Each tokenizer object should implement `LanguageTokenizer` directly:

```kotlin
internal object ExampleTokenizer : LanguageTokenizer {
    override fun tokenize(request: TokenizeRequest): List<SyntaxTokenSpan> {
        // Choose scanner mechanics, pass language-owned vocabulary/options, and return spans.
    }
}
```

Return the bare `List<SyntaxTokenSpan>` the scanner produces. Do not reintroduce a result wrapper or a routing adapter object. Register the tokenizer object itself in `routing/BuiltInTokenizers.kt`.

### 4. Map Keyword Categories By Meaning

Map tokens by the role SyntaxMP should expose, not by another tool's category name. Start the vocabulary audit in `builtins/<language>/<Language>Lexicon.kt`. Tokenizers and scanners should consume lexicon values for semantic word tables rather than keeping private keyword, constant, directive, or alias sets in implementation files.
...
### 6. Write Focused Fixtures

Place fixture tests under:

```text
syntaxmp-tokenizer/src/commonTest/kotlin/com/gallatinapps/syntaxmp/builtins/fixtures/<language>/<Language>FixtureTest.kt
```
...
### 8. Iterate And Record Divergence
...
```kotlin
package com.gallatinapps.syntaxmp.builtins.fixtures.kotlin
...
- Aliases were compared against the `BuiltInAliases` map in `LanguageId.kt`.
...
- `:syntaxmp-tokenizer:jvmTest` passes.
