...
class LanguageAliasSmokeTest {
    ...
    fun componentLanguageAliasesResolve() {
        ...
        val mdxCode = "# Metrics\n<Chart value={count} />"
        val vueCode = "<template>{{ count }}</template>"
        val svelteCode = "{#if count}<button>{count}</button>{/if}"
        val astroCode = "---\nconst title = \"Hi\";\n---\n<h1>{title}</h1>"
        ...
        assertTrue(defaultTestEngine().tokenize(mdxCode, "mdx").has(mdxCode, SyntaxRole.Tag, "Chart"))
        assertTrue(defaultTestEngine().tokenize(vueCode, "vue").has(vueCode, SyntaxRole.Variable, "count"))
        assertTrue(defaultTestEngine().tokenize(svelteCode, "svelte").has(svelteCode, SyntaxRole.Keyword, "{#if"))
        assertTrue(defaultTestEngine().tokenize(astroCode, "astro").has(astroCode, SyntaxRole.Markup, "---"))
    }
    ...
    fun yamlAndShellAliasesResolve() {
        ...
        val shell = defaultTestEngine().tokenize(shellCode, "bash")
    ...
    fun xmlAliasesResolveAndHighlightDeclarationsCdataAndAttributes() {
        ...
        """.trimIndent()
        val xml = defaultTestEngine().tokenize(code, "svg")
    ...
    @Test
    fun iniAndJson5AliasesResolveAndHighlightConfigShape() {
        val iniCode = """
        ...
        """.trimIndent()
        val ini = defaultTestEngine().tokenize(iniCode, "properties")
        val json5 = defaultTestEngine().tokenize(json5Code, "json5")
        ...
        assertTrue(json5.has(json5Code, SyntaxRole.Comment, "// relaxed config"))
        assertTrue(json5.has(json5Code, SyntaxRole.Property, "name"))
        assertTrue(json5.has(json5Code, SyntaxRole.String, "'NoteKit'"))
        assertTrue(json5.has(json5Code, SyntaxRole.Number, "0x2A"))
    }
    ...
    @Test
    fun terraformAndGraphqlAliasesResolveAndHighlightDomainShape() {
        val terraformCode = """
            # local note artifact
        ...
        """.trimIndent()
        val terraform = defaultTestEngine().tokenize(terraformCode, "tf")
        val graphQl = defaultTestEngine().tokenize(graphQlCode, "gql")

        assertTrue(terraform.has(terraformCode, SyntaxRole.Comment, "# local note artifact"))
        assertTrue(terraform.has(terraformCode, SyntaxRole.Keyword, "resource"))
        assertTrue(terraform.has(terraformCode, SyntaxRole.Property, "filename"))
        assertTrue(terraform.has(terraformCode, SyntaxRole.Function, "jsonencode"))
        assertTrue(terraform.has(terraformCode, SyntaxRole.Constant, "true"))
    ...
    @Test
    fun objectiveCLuaAndRAliasesResolve() {
        val objectiveCCode = """
        ...
        """.trimIndent()
        val rCode = """
            # summarize pinned rows
            score <- function(job) {
              total <- sum(job${'$'}count, na.rm = TRUE)
              if (is.na(total)) return(0)
              total
            }
            data.frame(name = c("scan", "archive"), count = c(4, NA))
        """.trimIndent()
        val objectiveC = defaultTestEngine().tokenize(objectiveCCode, "objc")
        val lua = defaultTestEngine().tokenize(luaCode, "lua")
        val r = defaultTestEngine().tokenize(rCode, "r")

        assertTrue(objectiveC.has(objectiveCCode, SyntaxRole.Annotation, "#import <Foundation/Foundation.h>"))
        assertTrue(objectiveC.has(objectiveCCode, SyntaxRole.Annotation, "@interface"))
        assertTrue(objectiveC.has(objectiveCCode, SyntaxRole.Type, "NSString"))
        assertTrue(objectiveC.has(objectiveCCode, SyntaxRole.Function, "NSLog"))
        assertTrue(lua.has(luaCode, SyntaxRole.Comment, "--[[ module comment ]]"))
        assertTrue(lua.has(luaCode, SyntaxRole.Keyword, "function"))
        assertTrue(lua.has(luaCode, SyntaxRole.Function, "score"))
        assertTrue(lua.has(luaCode, SyntaxRole.Constant, "true"))
        assertTrue(lua.has(luaCode, SyntaxRole.Function, "insert"))
        assertTrue(r.has(rCode, SyntaxRole.Comment, "# summarize pinned rows"))
        assertTrue(r.has(rCode, SyntaxRole.Keyword, "function"))
        assertTrue(r.has(rCode, SyntaxRole.Function, "sum"))
        assertTrue(r.has(rCode, SyntaxRole.Constant, "TRUE"))
        assertTrue(r.has(rCode, SyntaxRole.Property, "count"))
    }

    @Test
    fun scalaAndElixirAliasesResolve() {
        val scalaCode = """
            import scala.concurrent.Future
            final case class Job(name: String, count: Int, enabled: Boolean = true)
            object Jobs {
              def score(job: Job): Int =
                if job.enabled then math.max(0, job.count * 2) else 0
            }
        """.trimIndent()
        val elixirCode = """
            # worker scoring
            defmodule NoteKit.Job do
              @moduledoc "Scores %{name: name} jobs"
        ...
        """.trimIndent()
        val scala = defaultTestEngine().tokenize(scalaCode, "sc")
        val elixir = defaultTestEngine().tokenize(elixirCode, "ex")

        assertTrue(scala.has(scalaCode, SyntaxRole.Keyword, "case"))
        assertTrue(scala.has(scalaCode, SyntaxRole.Type, "Job"))
        assertTrue(scala.has(scalaCode, SyntaxRole.Function, "score"))
        assertTrue(scala.has(scalaCode, SyntaxRole.Type, "Int"))
        assertTrue(scala.has(scalaCode, SyntaxRole.Constant, "true"))
        assertTrue(elixir.has(elixirCode, SyntaxRole.Comment, "# worker scoring"))
        assertTrue(elixir.has(elixirCode, SyntaxRole.Keyword, "defmodule"))
        assertTrue(elixir.has(elixirCode, SyntaxRole.Annotation, "@moduledoc"))
        assertTrue(elixir.has(elixirCode, SyntaxRole.Type, "NoteKit"))
        assertTrue(elixir.has(elixirCode, SyntaxRole.Constant, ":ok"))
    }
    ...
    @Test
    fun perlAliasesResolve() {
        val perlCode = "sub score { my (\$job) = @_; print \$job->{name}; return 1; }"
        val perl = defaultTestEngine().tokenize(perlCode, "pl")

        assertTrue(perl.has(perlCode, SyntaxRole.Keyword, "sub"))
        assertTrue(perl.has(perlCode, SyntaxRole.Variable, "\$job"))
    }
