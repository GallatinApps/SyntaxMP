...
class HtmlFixtureTest {
    ...
    @Test
    fun `style lang scss body is tokenized as scss`() {
        val code = """<style lang="scss">${'$'}primary: #fff; .card { color: ${'$'}primary; }</style>"""

        assertTokenAt("html", code, "lang", "Attribute")
        assertTokenAt("html", code, "\"scss\"", "String")
        assertTokenAt("html", code, "${'$'}primary", "Variable")
        assertTokenAt("html", code, ".card", "Attribute")
        assertTokenAt("html", code, "color", "Property")
        assertTokenAt("html", code, "${'$'}primary", "Variable", occurrence = 1)
    }

    @Test
    fun `style lang sass body is tokenized as scss`() {
        val code = """<style lang="sass">${'$'}primary: #fff; .card { color: ${'$'}primary; }</style>"""

        assertTokenAt("html", code, "lang", "Attribute")
        assertTokenAt("html", code, "\"sass\"", "String")
        assertTokenAt("html", code, "${'$'}primary", "Variable")
        assertTokenAt("html", code, ".card", "Attribute")
        assertTokenAt("html", code, "color", "Property")
        assertTokenAt("html", code, "${'$'}primary", "Variable", occurrence = 1)
