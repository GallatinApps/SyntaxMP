...
internal class DockerfileScanner(
    ...
    private fun scanHeredoc(
        ...
        val routeToShell = shouldRouteShellHeredoc(
            instruction = instruction,
    ...
    private fun emitHeredoc(
        ...
        tokens.appendEmbeddedSpans(
            ...
            bodyStart = bodyStart,
            bodyEnd = bodyEnd,
            languageLabel = SyntaxLanguageId.Shell.value,
    ...
    private fun shouldRouteShellHeredoc(
        instruction: String,
        argumentStart: Int,
        heredocStart: Int,
        lineEnd: Int,
        ...
        if (instruction != "RUN") return false
        val before = commandTokens(argumentStart, heredocStart)
        val after = commandTokens(header.afterDelimiter, lineEnd)
        return when {
            before.isEmpty() && after.isEmpty() -> true
            before.isEmpty() && after.singleOrNull()?.isShellInterpreter() == true -> true
            before.singleOrNull()?.isShellInterpreter() == true && after.isEmpty() -> true
            else -> false
        }
    }
    ...
    private fun String.isShellInterpreter(): Boolean =
        this == "sh" || this == "bash"
