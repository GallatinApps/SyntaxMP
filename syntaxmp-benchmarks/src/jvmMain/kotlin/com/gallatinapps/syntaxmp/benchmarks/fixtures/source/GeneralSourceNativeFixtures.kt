...
// General-source fixtures for compiled-to-native languages (C, C++, Objective-C, Swift, Go, Rust,
// Dart). Grouped by runtime category only to keep files manageable; all are aggregated in
...
internal val ObjectiveCRepresentativeFixture = LanguageBenchmarkFixture(
    fixtureId = "objective-c",
    ...
    body = """
        ...
        typedef NS_ENUM(NSInteger, HJNoteStatus{{index}}) {
            HJNoteStatus{{index}}Draft,
            HJNoteStatus{{index}}Review,
            HJNoteStatus{{index}}Published,
        };

        @interface HJNote{{index}} : NSObject
        @property (nonatomic, copy) NSString *identifier;
        @property (nonatomic, copy) NSString *title;
        @property (nonatomic, copy) NSString *path;
        @property (nonatomic) NSInteger words;
        @property (nonatomic, getter=isPinned) BOOL pinned;
        @property (nonatomic) HJNoteStatus{{index}} status;
        - (instancetype)initWithIdentifier:(NSString *)identifier
                                     title:(NSString *)title
                                      path:(NSString *)path
                                     words:(NSInteger)words
                                    pinned:(BOOL)pinned
                                    status:(HJNoteStatus{{index}})status;
        - (NSInteger)readingMinutes;
        - (NSString *)label;
        @end

        @implementation HJNote{{index}}
        - (instancetype)initWithIdentifier:(NSString *)identifier
                                     title:(NSString *)title
                                      path:(NSString *)path
                                     words:(NSInteger)words
        ...
        - (NSInteger)readingMinutes {
            return MAX(1, (self.words + 219) / 220);
        }

        - (NSString *)label {
            return [NSString stringWithFormat:@"%@ / %ld min", self.title, (long)[self readingMinutes]];
        }
        @end

        @interface HJNoteQueue{{index}} : NSObject
        - (instancetype)initWithNotes:(NSArray<HJNote{{index}} *> *)notes;
        - (NSArray<HJNote{{index}} *> *)selectStatus:(HJNoteStatus{{index}})status;
        @end

        @implementation HJNoteQueue{{index}} {
            NSArray<HJNote{{index}} *> *_notes;
        }
        - (instancetype)initWithNotes:(NSArray<HJNote{{index}} *> *)notes {
            self = [super init];
            if (self) {
        ...
        - (NSArray<HJNote{{index}} *> *)selectStatus:(HJNoteStatus{{index}})status {
            NSPredicate *predicate = [NSPredicate predicateWithBlock:^BOOL(HJNote{{index}} *note, NSDictionary *bindings) {
                return note.status == status;
            }];
