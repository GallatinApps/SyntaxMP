...
private val HclRepresentativeFixture = LanguageBenchmarkFixture(
    fixtureId = "hcl",
    languageLabel = "hcl",
    displayName = "HCL",
    family = LanguageBenchmarkFamily.QuerySchema,
    targetBodyLines = 80,
    body = """
        variable "library_root_{{index}}" {
          type        = string
          description = "Root folder for benchmark library {{index}}"
          default     = "~/Notes/Library{{index}}"
        }

        variable "scan_limits_{{index}}" {
          type = object({
            readable_bytes      = number
            enhanced_edit_bytes = number
            metadata_only_bytes = number
          })
          default = {
        ...
        locals {
          library_name_{{index}} = "Library {{index}}"
          include_globs_{{index}} = [
            "**/*.md",
            "**/*.markdown",
            "**/*.txt",
            "projects/**/*.json",
          ]
          exclude_globs_{{index}} = [
            "archive/**",
            ".hashjot/**",
            "exports/tmp/**",
          ]
        ...
        scan "library_{{index}}" {
          name   = local.library_name_{{index}}
          root   = var.library_root_{{index}}
          limits = var.scan_limits_{{index}}

          patterns {
            include = local.include_globs_{{index}}
            exclude = local.exclude_globs_{{index}}
          }
...
private val TerraformRepresentativeFixture = LanguageBenchmarkFixture(
    fixtureId = "terraform",
    languageLabel = "terraform",
    ...
    body = """
        ...
        variable "environment_{{index}}" {
          type        = string
          description = "Environment name for fixture {{index}}"
          default     = "preview"
        }

        locals {
          name_prefix_{{index}} = "hashjot-${'$'}{var.environment_{{index}}}-{{index}}"
          common_tags_{{index}} = {
            Application = "HashJot"
            Owner       = "docs"
            Fixture     = "{{index}}"
        ...
        resource "aws_s3_bucket" "library_{{index}}" {
          bucket = "${'$'}{local.name_prefix_{{index}}}-library"
          tags   = local.common_tags_{{index}}
        }

        resource "aws_s3_bucket_versioning" "library_{{index}}" {
          bucket = aws_s3_bucket.library_{{index}}.id
          versioning_configuration {
            status = "Enabled"
          }
        }

        resource "aws_iam_policy" "reader_{{index}}" {
          name        = "${'$'}{local.name_prefix_{{index}}}-reader"
          description = "Read benchmark library {{index}} objects"
          policy = jsonencode({
            Version = "2012-10-17"
            Statement = [
              {
                ...
                Resource = [
                  aws_s3_bucket.library_{{index}}.arn,
                  "${'$'}{aws_s3_bucket.library_{{index}}.arn}/*",
                ]
              }
            ]
        ...
        output "library_bucket_{{index}}" {
          value       = aws_s3_bucket.library_{{index}}.bucket
          description = "Benchmark library bucket"
        }
    """,
)
...
internal val QueryInfrastructureFixtures: List<LanguageBenchmarkFixture> =
    listOf(
        ...
        HclRepresentativeFixture,
        TerraformRepresentativeFixture,
    )
